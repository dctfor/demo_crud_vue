  ...
  >
    <div class="card h-100 list-group-item-light a-s-box shadow">
      ...
      <div class="card-body">
        ...
        <ul class="list-group">
          <li class="list-group-item text-wrap">
            <b>Name:</b>
            <span class="fw-bold" :id="'spanName_' + contact.id">{{
              contact.name
            }}</span>
          </li>
          <li class="list-group-item text-wrap">
            <b>Email:</b>
            <span class="fw-bold" :id="'spanEmail_' + contact.id">{{
              contact.email
            }}</span>
          </li>
          <li class="list-group-item text-wrap">
            <b>Mobile:</b>
            <span class="fw-bold" :id="'spanMobile_' + contact.id">{{
              contact.mobile
            }}</span>
          </li>
        </ul>
      ...
      <div class="card-footer d-flex overflow-hidden" role="group">
        <button
          class="btn btn-theme w-100 c_f_btn"
          type="button"
        ...
        <button
          class="btn btn-theme w-100 c_f_btn_delete"
          type="button"
  ...
  methods: {
    ...
    editContact: async function (contact) {
      ...
      try {
        ...
        document.getElementById("modalEditDepartment").value =
    ...
    viewContact: async function (contact) {
      ...
      try {
        await axios
          .get(
            this.$apiUrl + this.$apiRoute + "contacts/" + contact.id,
            this.config
          )
          .then((response) => {
        ...
        document.getElementById("modalInfoCompany").value =
          this.contact.company;
        document.getElementById("modalInfoTitle").value = this.contact.title;
        document.getElementById("modalInfoDepartment").value =
