<template>
  <div>
    ...
    <MainModals
      @signalEdit="submitUpdate"
      @signalDelete="deleteContact"
      @signalNew="createContact"
    />
  </div>
</template>
...
export default {
  ...
  beforeMount: async function () {
    ...
    if (this.bErr) {
      ...
      setInterval((window.location.href = "/"), 7000);
    }
  },
  methods: {
    /* eslint-disable */
    ...
    submitUpdate: async function () {
      ...
      try {
        ...
        this.contact.company =
          document.getElementById("modalEditCompany").value;
        this.contact.title = document.getElementById("modalEditTitle").value;
        this.contact.departmentId = document.getElementById(
          "modalEditDepartment"
        ).value;
        ...
        await axios
          ...
          .then((response) => {
            document.getElementById("spanName_" + this.contact.id).textContent =
              this.contact.name;
            document.getElementById(
              "spanEmail_" + this.contact.id
            ).textContent = this.contact.email;
            document.getElementById(
              "spanMobile_" + this.contact.id
            ).textContent = this.contact.mobile;
            this.$toast.open({
